    ...
    do {
        ...
        if(isTcpMPEGTS) {
            ...
            if ((source = gst_element_factory_make("rtspsrc", "source")) != nullptr) {
                g_object_set(static_cast<gpointer>(source), "location", qPrintable(uri), "latency", 17, "udp-reconnect", 1, "timeout", _udpReconnect_us, NULL, nullptr);
            }
...
//     g_object_set(_videoSink, "sync", _buffer >= 0, NULL, nullptr);
...
GstVideoReceiver::_addVideoSink(GstPad* pad)
{
    ...
    // Create videoconvert + capsfilter
    GstElement* convert = gst_element_factory_make("videoconvert", "convert");
    GstElement* capsfilter = gst_element_factory_make("capsfilter", "capsfilter");

    GstCaps* rgbaCaps = gst_caps_new_simple("video/x-raw",
                                            "format", G_TYPE_STRING, "RGBA",
                                            nullptr);
    g_object_set(capsfilter, "caps", rgbaCaps, nullptr);
    gst_caps_unref(rgbaCaps);

    // Add elements to pipeline
    gst_object_ref(_videoSink); // gst_bin_add steals one ref
    gst_bin_add_many(GST_BIN(_pipeline), convert, capsfilter, _videoSink, nullptr);

    // Link: decoder → convert → capsfilter → videoSink
    if (!gst_element_link_many(_decoder, convert, capsfilter, _videoSink, nullptr)) {
        gst_bin_remove_many(GST_BIN(_pipeline), convert, capsfilter, _videoSink, nullptr);
        qCCritical(VideoReceiverLog) << "Unable to link decoder → convert → capsfilter → videoSink";
        if (caps != nullptr) {
            gst_caps_unref(caps);
        }
        return false;
    ...
    GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(_pipeline), GST_DEBUG_GRAPH_SHOW_ALL,
                              "pipeline-with-videoconvert-rgba");
...
GstVideoReceiver::_onBusMessage(GstBus* bus, GstMessage* msg, gpointer data)
{
    ...
    case GST_MESSAGE_ELEMENT:
        do {
            ...
            gst_structure_get(s, "message", GST_TYPE_MESSAGE, &forward_msg, NULL, nullptr);
