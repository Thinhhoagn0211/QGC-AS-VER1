...
const QVariantList& PX4FirmwarePlugin::toolIndicators(const Vehicle* vehicle)
{
    if (_toolIndicatorList.size() == 0) {
        ...
        for (int i=0; i<_toolIndicatorList.size(); i++) {
            if (_toolIndicatorList.at(i).toUrl().toString().contains("qrc:/qml/QGroundControl/Controls/FlightModeIndicator.qml")) {
        ...
        for (int i=0; i<_toolIndicatorList.size(); i++) {
            if (_toolIndicatorList.at(i).toUrl().toString().contains("qrc:/qml/QGroundControl/Toolbar/BatteryIndicator.qml")) {
